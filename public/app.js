...
const productSlug = params.get("product") || "test-product";
const requestedVariantId = params.get("variant") || null;
...
async function loadProduct() {
  try {
    ...
    if (!res.ok) throw new Error(data.error || "Product not available");

    product = data;
    variant = requestedVariantId ? product.variants.find((v) => v.id === requestedVariantId) : null;

    const displayName = variant ? `${product.name} — ${variant.name}` : (product.checkoutTitle || product.name);
    const meta = [
      variant ? `Qty: ${variant.quantity}` : null,
      product.freeShipping ? "Free shipping" : product.shippingLabel,
    ]
      .filter(Boolean)
      .join(" · ");
    const price = variant ? variant.price : product.price;
    const imageUrl = product.checkoutImage || product.imageUrl;
    ...
    if (imageUrl) {
      $("#s1-product-thumb").innerHTML = `<img src="${imageUrl}" alt="">`;
      $("#s2-product-thumb").innerHTML = `<img src="${imageUrl}" alt="">`;
    }
  } catch (err) {
    $("#s1-product-name").textContent = "Product not available";
    $("#s1-product-price").textContent = "";
    $("#step1-btn").disabled = true;
    console.error(err);
  }
}
...
function goToPayment() {
  ...
  customerData = {
    ...
    country: $("#cust-country").value,
  };

  showPage("step2");
}

// ── Payment flow (always credit card, via Mercuryo) ─────────────────
async function startPayment() {
  ...
  try {
    ...
    // 4. Redirect to the secure payment partner (Mercuryo)
